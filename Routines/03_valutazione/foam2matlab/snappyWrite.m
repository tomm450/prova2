function [done] = snappyWrite(nome_out,suffisso,SNAPPY_par,x_TE,featureAngle,case_dir)

  fid = fopen(sprintf('%s/10snappy/system/snappyHexMeshDict',case_dir),'w+');
  ...
  fprintf(fid,'castellatedMesh true; \n');
  fprintf(fid,'snap            true; \n');
  fprintf(fid,'addLayers       true; \n');
 ...
 fprintf(fid,'nMedialAxisIter 10; \n'); %// default: 2^31 (unlimited)
  %
  %     //- When not to extrude surface. 0 is flat surface, 90 is when two faces
  %     //  make straight angle.
  fprintf(fid,'    featureAngle %d;  \n',featureAngle);
  %     //- Maximum number of snapping relaxation iterations. Should stop
  %     //  before upon reaching a correct mesh.
  fprintf(fid,'    nRelaxIter 5; \n');
  ...
  %     // distance is large
  fprintf(fid,'    maxThicknessToMedialRatio 0.02; \n');
  %     // Angle used to pick up medial axis points
  %     // Note: changed(corrected) w.r.t 17x! 90 degrees corresponds to 130 in 17x.
  fprintf(fid,'    minMedianAxisAngle 90; \n');
  %     // Create buffer region for new layer terminations
  fprintf(fid,'    nBufferCellsNoExtrude 1; \n');
