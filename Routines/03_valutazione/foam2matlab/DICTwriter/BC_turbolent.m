function [done] = BC_turbolent(k_inlet,omega_inlet,case_dir,SOLVER,BU_par)

if strcmp(SOLVER.T_model,'sa')
    
    
    if strcmp(SOLVER.solver,'simple')
        fid = fopen(sprintf('%s/30simple/0/nuTilda',case_dir),'w+');
    else
        fid = fopen(sprintf('%s/40piso/0/nuTilda',case_dir),'w+');
    end
    ...
    fprintf(fid,'    object      nuTilda;  \n');
    fprintf(fid,'}  \n');
    fprintf(fid,'// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //  \n');
    
    fprintf(fid,'dimensions      [0 2 -1 0 0 0 0];  \n');
    ...
    fprintf(fid,'        freestreamValue uniform %f;  \n',BU_par.Nu/10);
